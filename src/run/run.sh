...
#           and return immediately to execute another commands without blocking it.
#    @file: run
...
if [ "$command" == "" ] ; then
	...
	echo "Bash script file to run any linux command or program with parameters from a terminal and return immediately to execute another without blocking it."
	echo "Sintaxe: run <program> [param 1] [param 2] ... [param 8]"
	echo "Examples:" 
	echo "   run gedit filename.txt"
	echo "   run firefox \"http://www.google.com\""
	echo "   run caja"
